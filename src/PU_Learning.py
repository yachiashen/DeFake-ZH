...
def train_initial_classifier(df_fake, df_trusted, test_size=0.2, random_state=RANDOM_STATE):
...
def pseudo_label_unlabeled(clf, tfidf, df_unlabeled, threshold=0.01):
...
def retrain_with_pseudo_labels(df_labeled, high_conf_neg, tfidf, random_state=RANDOM_STATE):
...
def main():
    ...
    fake_path = os.path.join(PROCESSED_DATA_FOLDER, "fake_ALL.csv")
    unverified_path = os.path.join(PROCESSED_DATA_FOLDER, "unverified_ALL.csv")

    df_fake, df_unverified = load_data(fake_path, unverified_path)

    # 初始標籤: cna & pts -> 真 (label=0)
    ...
    # 對無標籤資料做預測，挑出高信度真 (threshold=0.01)
    high_conf_neg, df_unlabeled_remainder = pseudo_label_unlabeled(
        clf, tfidf, df_unlabeled, threshold=0.01
    )

    # 第二輪訓練 (Pseudo-labeling)
    clf2, df_second_train = retrain_with_pseudo_labels(
        df_labeled, high_conf_neg, tfidf, random_state=RANDOM_STATE
    )
    ...
    # 輸出結果
    # 1) 第二輪後的標籤資料
    df_second_train.to_csv(os.path.join(PROCESSED_DATA_FOLDER, "PU_second_train.csv"), index=False, encoding="utf-8")

    # 2) 無標籤剩餘(尚未 pseudo-label) 的部分
    df_unlabeled_remainder.to_csv(os.path.join(PROCESSED_DATA_FOLDER, "PU_unlabeled_remainder.csv"), index=False, encoding="utf-8")

    # 3) 保留第一輪驗證集預測
    df_val.to_csv(os.path.join(PROCESSED_DATA_FOLDER, "PU_validation.csv"), index=False, encoding="utf-8")
    print("\n=== 處理完成！已輸出以下檔案至 data/processed/ ===")
    print("1) PU_second_train.csv")
    print("2) PU_unlabeled_remainder.csv")
    print("3) PU_validation.csv")
